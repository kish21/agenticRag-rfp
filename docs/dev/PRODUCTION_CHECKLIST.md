...
| `LANGSMITH_API_KEY`   | ✅ Set          | smith.langchain.com → Settings → API Keys                            |
| `COHERE_API_KEY`      | ✅ Set          | —                                                                    |
...
## 6. Evaluation Routes ✅ Complete

`app/api/evaluation_routes.py` is built and registered in `app/main.py`.
...
Pipeline execution (calling agents in sequence from `/start`) is the next step for full end-to-end runs.
