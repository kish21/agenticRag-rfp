...
| **E3** | Evidence quality / benchmark | 🟢 DONE + merged (#195) 2026-05-31 | repeatable ground-truth benchmark + baseline (grounding **1.00**, 0 fabricated); no-forced-scores done (insufficient-rate 0.00→0.80). Follow-ups: E3.a extraction-recall (in progress — found to be a benchmark-matcher artifact), E3.b contradiction→insufficient, E3.c missing-mandatory→reject, E3.d coverage-normalised ranking, E3.e gates+reranker, E3.f scanned. |
...
- **E3.a — extraction recall** ✅ mostly RESOLVED as a measurement error: the "0.60 / ~37% missed" was a benchmark matcher/golden artifact (punctuation-insensitivity + an over-specified cert `status` field), not a pipeline gap. Corrected matcher + golden → true recall **~0.80**. **Residual (real, kept failing):** `project` facts are genuinely missed (`extracted=0`) in table-heavy + sparse-doc layouts — a layout-sensitivity gap in retrieval-context/extraction worth a real fix; also extraction shows run-to-run variance (single-run estimate). Next: investigate why `project` extraction misses tables/short docs.
- **E3.b — contradiction → insufficient** 🟢 mostly DONE: extraction now surfaces conflicting values (prompt), `evaluate_check` + `evaluation.py` resolve a flagged contradiction to `insufficient_evidence` (no fallback mask), and the report now **completes** for contradicted/rejected vendors — the rejection + each conflict are injected as `system_facts` (trusted determinations, no PDF grounding needed; the honesty rule on PDF claims is untouched). Verified directly from the pipeline: the **insurance** £10M-vs-£2M conflict → `insufficient_evidence` with the conflict captured. **Residuals:**
  - **E3.b.1 — cert status-conflict ⏸️ DE-PRIORITISED (measure-first verdict 2026-06-01).** Re-ran the benchmark on `594067e` (`results_20260601T144942Z`): a `CONTRADICTED` cert-status schema/enum would move the number by **zero**, for two reasons. (a) The cert conflict is lost at **extraction** — cert recall 0.5, the extractor collapses the two ISO 27001 claims ("valid until 2027" / "lapsed 2023") into one row, so no conflict reaches the evaluator (overlaps E3.a layout-sensitivity). (b) Even if it did, epsilon's whole evaluation is **dropped before grading** (E3.b.2 below). The general value-contradiction path (#198) already handles numeric conflicts — insurance £10M/£2M both extracted (recall 1.0). **Revisit only after E3.b.2 + the block-vs-insufficient decision, and only if a cert-specific gap remains once a contradicted vendor reaches the grader.** Original (now-superseded) diagnosis: "extraction emits a single status so the evaluator never sees the conflict."
  - **E3.b.2 — benchmark grader can't read a blocked/retried evaluation** 🔼 PROMOTED to next-session lead (confirmed 2026-06-01 on `594067e`). In the full graph run, `05_conflicting/epsilon` evaluation **HARD-blocks → retries 3× → fails → vendor dropped** ("Scored 0 of 1"; comparator `empty_ranking`). `state_to_actual` reads decisions from `final_state["evaluation_output_objects"]` ([benchmark/runner/pipeline_adapter.py:139,153](benchmark/runner/pipeline_adapter.py#L139)), which is **empty** for the dropped vendor → both expected-insufficient checks score as "not insufficient" (`forced_when_insufficient=2`, `mandatory_accuracy=0.00`). The 0.00 is a **measurement artifact**, not a product bug (the mandatory-check fn resolves contradictions to `insufficient_evidence` — `tests/test_contradiction_insufficient.py` + direct capture). Three prior runner attempts (values-mode, retain-richest) didn't crack it. **Coupled design Q (decide with the fix):** should the eval critic HARD-block a contradicted vendor at all, or resolve it to `insufficient_evidence` and keep it in the report (#198 part-0 "report always completes")? — that determines what the grader should read.
...
- **E3.g — prompt hygiene + benchmark-driven refinement** (NEW): now that local YAML is the authoritative prompt source (PR — prompts-local-authoritative), refine prompts one at a time, re-measuring each. **Lead item — brace bug:** `get_prompt` does a targeted `str.replace`, NOT `str.format`, and does NOT collapse `{{`→`{`; yet `setup/interpret_criteria_sheet.yaml`, `setup/generate_score_guides.yaml`, `setup/suggest_mandatory_checks.yaml` use `{{ }}` in their JSON examples, so the LLM receives literal double-braces (malformed example). `extract_rfp_criteria.yaml` correctly uses single `{`. Normalise `{{`→`{` in those (safe — targeted replace), and verify `explanation/generate_narrative.yaml` (benchmark-path → re-measure). Found via /code-review during the prompt-source fix.
- **E3.h — benchmark crashes on Windows stdout (NEW, 2026-06-01):** `benchmark/runner/run_benchmark.py:67` prints `→ running … ` (U+2192/U+2026); Windows default cp1252 stdout raises `UnicodeEncodeError`, killing the run before any scenario. Workaround in use: `PYTHONUTF8=1 python -m benchmark.runner.run_benchmark`. Fix: reconfigure stdout to UTF-8 at runner start (or replace the unicode glyphs with ASCII). Trivial, cross-platform.
- **E3.i — benchmark may not honour `RERANKER_PROVIDER` (NEW, 2026-06-01):** `.env` had `RERANKER_PROVIDER=modal`, but the run repeatedly logged `Reranker 'bge' failed (no HF egress) — falling back to vector-score order`. So the committed baseline's retrieval is **unreranked** (recall still 1.00, so conclusions hold, but the number isn't a reranked number). Investigate whether the benchmark/pipeline path reads `RERANKER_PROVIDER`; ties into E3.e (fair retrieval number on a box with HF egress / via the Modal reranker).
...
### P2.25 — Grounding-completeness computation duplicated across explanation.py and nodes.py (code review 2026-06-01, PR #198) — ✅ DONE (2026-06-03)

The `claim_bearing` / `total_claims` / `grounded_claims` / `grounding_completeness` block (plus the methodology_note + limitations text) was byte-identical in `app/agents/explanation.py::run_explanation_agent` and `app/pipeline/nodes.py::explanation_finalise`. **Fixed:** extracted `compute_grounding(vendor_narratives) -> (completeness, total_claims, base_limitations)` + a `METHODOLOGY_NOTE` constant in `app/agents/explanation.py`; both paths now call it (callsites append only their stage-specific limitations). A drift guard test asserts the live path calls the helper and never re-introduces an inline `grounded_claims / total_claims`. **Provenance.** `/code-review` of PR #198 (E3.b).

### P2.26 — Vacuous `grounding_completeness=1.0` weakens the critic honesty gate for all-system-facts reports (code review 2026-06-01, PR #198) — ✅ DONE (2026-06-03)
...
### P2.27 — `run_cleanup` deletes a whole org's vectors when ANY one of its setups expires (pre-existing, surfaced by #215, 2026-06-04) — ✅ DONE (2026-06-05, PR #241)

`app/jobs/cleanup.py` deleted Qdrant data at **org granularity**: for each expired `evaluation_setups` row it removed every point with that `org_id`. A Qdrant point carried `org_id` / `vendor_id` / `rfp_id` / `doc_id` but **no `setup_id`**, so a single expired setup wiped the org's *other, still-live* setups too. This was a **pre-existing** behaviour (the old code did the same via `startswith("platform_{org_id}_")`), faithfully preserved by #215 (per-org collection) — NOT introduced by it. **Fixed:** chunks are stamped with `setup_id` at ingestion (`app/agents/ingestion.py`); new `delete_setup_data(org_id, setup_id)` (`app/retrieval/qdrant.py`) deletes only that setup's points and drops the per-org collection only when its **last** setup goes; `run_cleanup` calls it per expired setup. On a vector-delete failure the PostgreSQL row is **kept** (retry next run, no orphan) + a `failed_setups` counter. `delete_setup_data`/`delete_org_data` share `_delete_by_filter_and_maybe_drop`. Verified with in-memory Qdrant + SQLite **and live against the real Qdrant container** (throwaway self-cleaning org). `/security-review`: no findings (tenant-scoped). Doc: `docs/dev/E_P227_SETUP_PRECISION_CLEANUP.md`. **Residual (documented, accepted):** points ingested before this fix carry no `setup_id` and are not matched — bounded disposable test data, no prod org; an org-coarse fallback would re-introduce the bug. **Provenance.** Tracing #215 (E215).

### P2.28 — `_verdict()` infers ESCALATED from the word "escalate" in a HARD flag's free-text recommendation (surfaced by #133 injection defence, 2026-06-05)

**Problem.** `app/agents/critic.py::_verdict()` decides ESCALATED vs BLOCKED by string-matching `"escalate" in recommendation.lower()`. A HARD flag that must hard-block the pipeline (e.g. `prompt_injection_detected`) is silently downgraded to ESCALATED — which `app/pipeline/nodes.py::_hard_block_if()` lets through — if any maintainer rewords its recommendation to contain "escalate". The #133 flag guards this only with a code comment + a unit test asserting `verdict == BLOCKED` and `"escalate" not in recommendation`. **Fix.** Replace the string-inference with an explicit field on `CriticFlag` (e.g. `forces_block: bool` / `escalatable: bool`) so severity routing is data-driven, not prose-driven, across ALL hard flags. **Effort:** half a day (touches `_make_flag`, `_verdict`, every `_make_flag` call site, and `_hard_block_if`). **Provenance:** #133 code-review, 2026-06-05.
...
| P3.13 | Remove 3 dead imports in `app/pipeline/nodes.py` (`critic_after_comparator` L25, `critic_after_decision` L26, `run_explanation_agent` L36 — unused since #150, Pylance `reportUnusedImport`; editor-only, CI does not run Pylance/ruff) | 5 min |
| P3.14 | Replace deprecated `datetime.utcnow()` → timezone-aware `datetime.now(timezone.utc)` (26 occ, 13 files; warnings only, Python 3.11+). `import datetime` files (`app/providers/observability.py:143`, `app/jobs/rate_monitor.py:24`, `app/jobs/cleanup.py:18,63`) → `datetime.datetime.now(datetime.timezone.utc)`; `from datetime import datetime` files (`app/agents/decision.py:39,47`, `app/domain/override.py:33`, `app/api/evaluation_routes.py:858`, `app/infra/logger.py:58,80,127,141`, `tools/contract_tests.py:87,94,183`, `tools/checkpoint_runner.py:272,397,462`, `tests/unit/test_procurement_agent.py:86`, `tests/test_parallel_fanout.py:216`, `tests/regression/run_regression.py:193,222,308,337,466,478`) → `datetime.now(timezone.utc)`. **Care:** `logger.py:58,80` do `.isoformat() + "Z"` — aware `.isoformat()` already emits `+00:00`, so DROP the `+ "Z"` (else malformed `+00:00Z`). All target DB cols are `TIMESTAMPTZ` so aware is correct. Verify: full suite + `python -W error::DeprecationWarning`. Batch with any Python-version bump. (Audit finding A, 2026-06-03.) | Half a day |
