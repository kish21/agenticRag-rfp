...
- **E3.b — contradiction → insufficient** 🟢 mostly DONE: extraction now surfaces conflicting values (prompt), `evaluate_check` + `evaluation.py` resolve a flagged contradiction to `insufficient_evidence` (no fallback mask), and the report now **completes** for contradicted/rejected vendors — the rejection + each conflict are injected as `system_facts` (trusted determinations, no PDF grounding needed; the honesty rule on PDF claims is untouched). Verified directly from the pipeline: the **insurance** £10M-vs-£2M conflict → `insufficient_evidence` with the conflict captured. **Residuals:**
  - **E3.b.1 — cert status-conflict not surfaced**: "valid" vs "expired/lapsed" is one cert with a contradicted *status*; extraction emits a single status, so the evaluator never sees the conflict (chk-iso still passes). Needs an extraction-schema way to represent a contradicted/uncertain status.
  - **E3.b.2 — benchmark grader under-reports the conflicting scenario**: `state_to_actual` reads `evaluation_output_objects` from the final state, but after the **evaluation critic-retry** that key is cleared from the top-level final state (the result lives in a nested context), so `mandatory_accuracy` shows 0.00 for `05_conflicting` even though the pipeline is correct. Three runner attempts (values-mode, retain-richest) did not crack it; reverted. The product is verified correct via direct capture; the grader needs to read retried-evaluation results from the right place.
