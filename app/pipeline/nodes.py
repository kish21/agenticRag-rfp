...
async def explanation_finalise(state: PipelineState) -> dict:
    ...
    try:
        ...
        # Grounding completeness = fraction of PDF claims that grounded, measured
        # ONLY over narratives that actually made PDF claims. A narrative whose
        # content is entirely system_facts (e.g. a vendor rejected for a conflict —
        # its story is a trusted system determination, not a PDF quote) has nothing
        # that needed grounding and must not be scored 0%. If no narrative made any
        # PDF claim, completeness is vacuously 1.0. The honesty rule stays fully
        # strict on the PDF claims that ARE made (a narrative that made claims which
        # all failed grounding still scores 0 and is blocked).
        claim_bearing = [
