...
def cp_auth() -> list[CP]:
    ...
    # A2: Token stored — either localStorage (old) or via setUserInfo (cookie-based auth)
    stores_token = (
        ("access_token" in login and "localStorage.setItem" in login) or
        "setUserInfo" in login
    )
    results.append(CP("A2", "Login stores JWT in localStorage.access_token", stores_token))

    # A3: Home page has auth guard — either direct token check or isLoggedIn()
    home = read_file("app/page.tsx")
    has_guard = (("access_token" in home or "isLoggedIn" in home) and "/login" in home)
