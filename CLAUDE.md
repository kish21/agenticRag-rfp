...
**#221 (pytest 8→9) DONE — committed LOCAL on branch `221-pytest-9` (commit `13a6bf4`, 2026-06-05, STACKED on #222); NOT pushed (evening batch).** Bumped `pytest 8.3.5→9.0.3` (fixes `CVE-2025-71176`) + `pytest-asyncio 0.25.3→1.4.0` (0.25.x capped `pytest<9`; 1.4.0 allows `pytest<10,>=8.4`, needs Python≥3.10 — CI is 3.11, local 3.13). Pinned `asyncio_default_fixture_loop_scope = "function"` in `pyproject.toml` so the 1.x default is explicit. **Removed the LAST `ignore-vulns` from `.github/workflows/ci.yml` → the `pip-audit` gate now runs with ZERO ignores** (closes the #220→#222→#221 security-baseline thread; policy comment now says "fix at source, never re-add an ignore"). Files: `requirements-dev.txt`, `pyproject.toml`, `.github/workflows/ci.yml`, `CHANGELOG.md`, `docs/dev/E221_PYTEST_9.md`. **Verified by RE-RUNNING the whole suite across the pytest-asyncio 1.0 rewrite** (a pin you can't watch fail isn't verified): **305 passed, 3 skipped** (identical to pre-bump), 308 collected, no removed/legacy API hit, `asyncio_mode=auto` still valid. `pip-audit` clean with NO ignores on BOTH `requirements.txt` + `requirements-dev.txt`; contracts 14/14; drift OK. No app code touched → no `/security-review` (dev-only dep). Cross-checked CI installs both reqs (ci.yml:153) on py3.11 so the bump takes effect. See [[feedback_evening_batch_push]].

**P2.27 (per-setup retention precision) DONE — committed LOCAL on branch `p227-setup-precision-cleanup` (commit `f243c3f`, 2026-06-05, STACKED on #221); NOT pushed (evening batch).** Cleanup deleted Qdrant data at ORG granularity, so one expired `evaluation_setups` row wiped the org's other still-live setups (a point carried no `setup_id`). Fix: stamp `setup_id` on chunk payloads at ingestion ([app/agents/ingestion.py](app/agents/ingestion.py) — `EvaluationSetup` already in scope, no signature change); new `delete_setup_data(org_id, setup_id)` in [app/retrieval/qdrant.py](app/retrieval/qdrant.py) deletes only that setup's points + drops the per-org collection only when its LAST setup goes; [app/jobs/cleanup.py](app/jobs/cleanup.py) calls it per expired setup (was `delete_org_data` per org). **Correctness link verified:** the stamped `setup_id` == `evaluation_setups.setup_id` PK (persisted in `fact_store.py`), which cleanup deletes by. This was the ONE #215 piece NOT live-tested (exercising it deletes data) — covered with **in-memory Qdrant + SQLite** (never touches real data). **`/code-review` (medium, 2 agents) fixed 2:** (1) on Qdrant-delete failure cleanup now KEEPS the PG row (retry next run) instead of orphaning vectors + a `failed_setups` counter; (2) DRY'd `delete_setup_data`/`delete_org_data` behind `_delete_by_filter_and_maybe_drop` (mirrors `_tenant_must`). Deliberately NOT actioned: pre-P2.27 unstamped points (bounded/disposable, no prod data; an org-coarse fallback would re-introduce the bug — documented). **`/security-review`: NO findings** (tenant-scoped to the per-org collection + org_id/setup_id filter from trusted DB rows; parameterized; no new auth surface). +7 tests (`tests/test_cleanup_setup_precision.py`); full suite **312 passed, 3 skipped**; contracts 14/14; drift OK. `delete_org_data` retained as the whole-tenant GDPR-erasure primitive (caller-less, like `delete_vendor_data`). Doc: `docs/dev/E_P227_SETUP_PRECISION_CLEANUP.md`. See [[feedback_evening_batch_push]].

**Next action (next session): pick one.** **E3.f (#209 scanned/OCR — P4)**, **E3.g**, **E2 auth follow-ups**, **8b** (delivery completion hook + Mode C auto-trigger; needs SMTP creds). (E3.b.1 residual = #210, de-prioritised.) NOTE: FOUR local commits this session (`af58f86` #222, `c8b732f` handoff, `13a6bf4` #221, `0e36dd4` handoff) + P2.27 (`f243c3f`) await the evening batch — three subtasks built same-day at the user's explicit direction ("you can start here" / "ATART p2.27"), the exception to one-subtask-per-session.
