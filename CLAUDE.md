...
**Next action (next session): START HERE → do issue #212 (reranker air-gapped default).** Config-only, high-value, offline-testable: a defaulted org (no `org_settings` row) resolves to the product default `reranker_provider=bge` ([product.yaml:38](app/config/product.yaml#L38) via `_defaults_for`), which **overrides** `.env`'s `RERANKER_PROVIDER=modal` and attempts a local HuggingFace download — bad for air-gapped VPCs (silently degrades to vector order). Fix is the **config default** (set product.yaml/preset/loader default to `modal` or `none`; align all default surfaces), NOT the reranker.py fallback (already `none`; the external audit's "edit reranker.py" advice was a misread). Add a test that a default `OrgSettings` resolves to the safe provider; decide fail-open-vs-fail-closed. See [[project_reranker_env_dead_config]]. **Then** (lower): #215 (Qdrant per-org collection redesign — P2, when a collection-count trigger nears), E3.f (#209), E2 auth, 8b.
