...
**Next action (next session):** E3.b.2 is DONE (PR #200, awaiting merge). Next lever options (pick one): **(a) E3.c — missing-mandatory → reject** (the `06_missing_evidence/omega` scenario still shows `rejection_correct=False` in BOTH pre/post-#200 runs — a real, pre-existing gap: a vendor missing a mandatory item is not being rejected); **(b) the benchmark grader should still represent a *genuinely* blocked/failed vendor distinctly** (read `failed_vendors`/`blocked` in `state_to_actual`) so a real future HARD-block (e.g. fabrication) is never silently mis-scored as "not insufficient" — #200 removed the contradiction-driven drop but the grader blind-spot for other drops remains; **(c) E3.b.1 cert-extraction collapse** — only worth it if a cert-specific gap matters once (a) is done (the extractor merges epsilon's two ISO 27001 rows → cert recall 0.5). **E3.b.1 stays DE-PRIORITISED.** Lower priority: E3.d-g, E2 auth follow-ups, P2.25/P2.26 cleanups, 8b. **Recommend (b) or (a)** — both are now the measured frontier.
