...
**🔴 Step 1 — ROOT CAUSE FOUND: scoring starves because extraction targets are all `custom` (design drift).** Symptom: on the REAL fixtures (`data/documents/` — RFP + Acme/ClearPath + Apex + criteria CSV) the pipeline runs end-to-end but both vendors score only **~0.2-0.3/10**. **First read the user's design docs `docs/dev/AGENT_04_EXTRACTION.md` + `AGENT_05_EVALUATION.md`** — the impl drifted from them. **The bug:** `app/domain/criteria.py::_build_targets()` (≈L661-687) hardcodes EVERY extraction target to `fact_type:"custom"` (L671 mandatory, L682 scoring) — no inference of typed fact_type from the criterion. Per AGENT_04 targets must be typed. Consequence chain (verified): all-custom → `app/agents/_extraction/prompts.py::_schema_description` only asks LLM for *custom* facts → typed PostgreSQL tables (certs/insurance/sla/project/pricing) never fill → `app/agents/evaluation.py::_get_facts_for_target` (≈L201-213) returns `[]` for typed criteria → `_score_criterion` → insufficient/0 → low scores. Also `_extraction/retry_loop.py` (≈L60) only retries typed fact_types so it never fires. **This is why "it scored well before agentic."** NOT a calc bug, NOT from E3.b (E3.b didn't touch `_build_targets`).
  - **THE FIX:** restore typed-target inference in `_build_targets()` per AGENT_04 — map criterion/check → fact_type ("ISO 27001"→certification, "Professional Indemnity"→insurance, "SLA/response time"→sla, "reference/project"→project, "pricing/commercial"→pricing, else custom). Decide: deterministic keyword heuristic vs have the criteria-extraction LLM emit fact_type. Then re-run smoke+benchmark; confirm typed facts populate + scores recover.
  - Secondary (optional, NOT the root fix): chunk-fallback for `_score_criterion` mirroring `_evaluate_mandatory_check`'s `should_fallback`. Plus verify likely **display scaling** of `total_weighted_score` (shown `/10`, looks 0-1).
  - Full design-vs-code drift report is in the plan file `~/.claude/plans/typed-splashing-lollipop.md`.
