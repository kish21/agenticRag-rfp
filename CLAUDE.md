...
**Next action (next session):** E3.b.2 (PR #200) + E3.c (PR #201, STACKED on #200) BOTH DONE, awaiting merge. **MERGE ORDER: merge #200 first, then `gh pr edit 201 --base master` and merge #201** (its base is the #200 branch so the diff stays clean). Next lever options (pick one): **(a) E3.b.1 cert-extraction collapse** — the extractor merges epsilon's two ISO 27001 rows → cert recall 0.5, which keeps epsilon's `mand` bouncing 0.5↔1.0 run-to-run; fix the extractor to KEEP both conflicting cert rows so the cert contradiction survives to evaluation. **This is now the measured frontier — post-#201 it's the last thing moving an aggregate number.** No longer "move by zero" (that was true only while epsilon was being dropped/mis-graded — #200/#201 changed the baseline). **(b) grader robustness** — `state_to_actual` should represent a *genuinely* blocked/failed vendor distinctly (read `failed_vendors`/`blocked`) so a real future HARD-block (e.g. fabrication) is never silently mis-scored; #200 removed the contradiction-driven drop but the blind-spot for OTHER drop causes remains; **(c) E3.d** coverage-normalised ranking (decision.py already flags insufficient criteria for it). Lower priority: E3.e-g, E2 auth follow-ups, P2.25/P2.26 cleanups, 8b. **Recommend (a).**
