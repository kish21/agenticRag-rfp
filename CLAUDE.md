...
**Next action (next session): pick one.** **Recommend #222 (jose→PyJWT)** or **#221 (pytest 9)** while fresh — both directly clear the 3 remaining CVE ignores (small, well-scoped, #222 is 3 files). Else: **P2.27** (stamp `setup_id` on chunk payloads so cleanup deletes precisely instead of org-coarse — small correctness win surfaced by #215), E3.f (#209 scanned/OCR — P4), E3.g, E2 auth follow-ups, 8b. (E3.b.1 residual = #210, de-prioritised.)
