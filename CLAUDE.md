...
**Current skill:** Phase 3 + Phase 5 done 2026-05-29. 2026-05-30: docs refresh (#172), real BM25/P1.12 (#173), Phase 7 plan align (#174), **Phase 7 customer report (#176)** + polish (#177) + **frontend report buttons (#178)**, **Phase 8a delivery channels (#179)** + **delivery service facade (#181)** + decomposition (#182), **Phase 2c exit-criteria contract (#183)** + **self-correcting retry engine (#184)**, P0/P1/P2 code-review fixes (#188), **Phase 2c wiring COMPLETE + merged (#189)**, **P0.16 tenant isolation / RLS now enforces + merged (#190)** + CI lint fix (#191). Phase 1, 2, 2c, 4, 7, 9 on master; Phase 8 module foundation on master. Enterprise-readiness: **E1 (tenant isolation) DONE**; E2 (auth) next. Remaining: 6, 8b wiring, 10.
**Last verified checkpoint:** **#190 + #191 merged (commit 3c4f84d).** P0.16 done: RLS genuinely enforces — non-superuser `platform_app` role (RLS-governed) + owner `platformuser` (RLS-exempt, DDL/auth/cron) split in `app/db/session.py`; `FORCE ROW LEVEL SECURITY` dynamically on every RLS-enabled table; index-friendly uuid policies (`org_id = NULLIF(current_setting('app.current_org_id',true),'')::uuid`); same-connection org context via pure-ASGI `OrgContextMiddleware` + pool checkout listener (set once per org-change); operational tables (rfps/ingestion_jobs/event_log/invited_vendors) brought under RLS; **no DB password in source** (CI-generated, `POSTGRES_APP_PASSWORD` injected) + **gitleaks** CI job; fail-loud `_check_db_app_role` startup guard; fixed latent `gaps_report` column. schema.sql + Alembic `0011`. Reviewer note `docs/dev/TENANT_ISOLATION.md`. Tests: `tests/test_tenant_isolation_rls.py` (13, incl. real-app-role E2E) + `tests/conftest.py`; full suite **204 green**; CI green; contracts 14/14; drift OK.
**Next action:** **E2 — Auth hardening** (BACKLOG `docs/dev/BACKLOG.md` E2, verified real bugs). Test-first, one PR: (1) env-aware `secure` cookie (`auth_routes.py:31` hardcodes `secure=False`); (2) reconcile `schema.sql UNIQUE(email)` vs `ON CONFLICT (email, org_id)` — multi-org signup breaks today (+Alembic); (3) session/token revocation table (JWT alone can't be revoked); (4) invite = one-time expiring token, add password-reset flow, no endpoint returns plaintext passwords; (5) happy+abuse tests + auth reviewer note. Then queued: E3 benchmark, P2.23/P2.24, 8b wiring; follow-ups — migrate functional suite to run as the app role, RLS on `organisations`/billing shells.
**Blockers:** none
