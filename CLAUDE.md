...
**Next action (next session): P2.27 — RECOMMENDED.** Stamp `setup_id` on chunk payloads so cleanup deletes precisely instead of org-coarse — a small correctness win surfaced by #215 (a point currently carries no `setup_id`, so one expired setup purges the org's live setups too). **Verify-first caution:** this is the ONE #215 piece NOT live-tested because exercising it deletes data — architect a safe test (e.g. a throwaway org) before touching `cleanup.py`. **Else:** E3.f (#209 scanned/OCR — P4), E3.g, E2 auth follow-ups, 8b. (E3.b.1 residual = #210, de-prioritised.) NOTE: three local commits (`af58f86` #222, `c8b732f` handoff, `13a6bf4` #221) await the evening batch push.
